...
<body>
    <div style="width:21.0cm;margin:0 auto; font-size:18px">
        ...
        <!--TITLE-->
        <h2 class="text-center">DAFTAR PENGELUARAN RIIL</h2>
        ...
        <p class="text-justify">
            ...
            bahwa saya bertanggung jawab secara formal dan material dan kebenaran perhitungan pemungutan pajak atas
            segala pembayaran tagihan yang telah kami perintahkan dalam SPM ini dengan perincian sebagai berikut :
        </p>
            ...
            <tbody>
                <tr>
                    ...
                    <td class="text-justify">
                        ...
                        @php
                        $dasar = [];
                        if (!empty($data->detail->dasar['isSK'] ?? false)) {
                        $dasar[] = 'SK No. ' . ($data->detail->nomor_sk ?? '-') . ' tanggal ' . ($data->detail->tanggal_sk_indonesia ?? '-');
                        }
                        if (!empty($data->detail->dasar['isKuitansi'] ?? false)) {
                        $dasar[] = 'Kuitansi No. ' . ($data->detail->kuitansi_nomor ?? '-') . ' tanggal ' . ($data->detail->tanggal_dokumen_indonesia ?? '-');
                        }
                        echo implode(' dan ', $dasar);
                        @endphp
                        .
                    </td>
                    <td class="text-center">{{ number_format($data->total ?? 0, 0, ',', '.') }}</td>
                    <td class="text-center">
                        @php
                        $ppn = collect($data->daftarNominal ?? [])->sum('ppn');
                        @endphp
                        {{ number_format($ppn ?? 0, 0, ',', '.') }}
                    </td>
                    <td class="text-center">
                        @php
                        $pph = collect($data->daftarNominal ?? [])->sum('pph');
                        @endphp
                        {{ number_format($pph ?? 0, 0, ',', '.') }}
                    </td>
                </tr>
            </tbody>
            <tfoot>
                <tr>
                    ...
                    <th>{{ number_format($ppn ?? 0, 0, ',', '.') }}</th>
                    <th>{{ number_format($pph ?? 0, 0, ',', '.') }}</th>
                </tr>
            </tfoot>
        </table>
        ...
        <p class="text-justify">
            ...
            Anggaran/Kuasa Pengguna Anggaran untuk kelengkapan administrasi dan pemeriksaan aparat pengawasan fungsional.
        </p>
        <p class="text-justify">
