...
class DaftarNominalController extends Controller
{
    ...
    public function index($id)
    {
        ...
        $data = DokumenPencairanSesi::with(['kegiatan', 'pelaksanaanDasar', 'daftarNominal' => function ($daftarNominal) {
            $daftarNominal->orderBy('urutan');
        }, 'kodeAkun', 'nominalPengaturan', 'ppk.pegawai', 'bendahara.pegawai'])->where('id', $id)->get();
        foreach ($data as $item) {
            ...
            $total = 0;
            $pajak = 0;
            foreach ($item->daftarNominal as $d) {
                $total = $total + $d->total;
                $pajak = $pajak + $d->pph;
            }
            $item->total = $total;
    ...
    public function store(Request $request)
    {
        // return $request->data;
        try {
            DaftarNominal::where('dokumen_pencairan_sesi_id', $request->sesi_id)->delete();
